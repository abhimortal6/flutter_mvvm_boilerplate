...
class ApiHelper {
  ...
  static Future<Map<String, String>> _getHeaders(
      bool useAuth, int apiVersion) async {
    Map<String, String> _map = {
      //Add or Remove headers from here
      'Content-Type': 'application/json',
      'Authorization': useAuth
          ? 'Bearer ' + (await SingletonConstants().getToken ?? "")
          : "",
    };
    return _map;
  }
...
//                if (responseData.message != null) await Constant().genericErrorDialog(context, responseData.message, 0);
  ...
  Future<ResponseData> postRequest(
      BuildContext? context, var requestUri, Map map,
    ...
    if (await NetworkCheck.isOnline(context, showConnectivityError)) {
      ...
      await http
          .post(requestUri,
              body: jsonBody, headers: await _getHeaders(useAuth, apiVersion))
        ...
        if (response.statusCode == 200 || response.statusCode == 201) {
          if (showLog) {
            debugPrint(
                "$responseName Response: ${prettyJson(jsonDecode(response.body))}");
          }
          responseData = ResponseData.fromResponse(response);
        } else {
          responseData = await _handleError(
              context, response, responseName, showLog, showMessage, showError);
        }
      }).catchError((error) {
        if (showLoader) LoaderWidget.hideLoader(context);
